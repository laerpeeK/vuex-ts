...
describe('Store', () => {
  ...
  it('commiting with object style', () => {
    const store = new Vuex.Store({
      state: {
        a: 1,
      },
  ...
  it('asserts the call with the new operator', () => {
    expect(() => {
      // @ts-expect-error
      Vuex.Store({})
    }).toThrowError(/Class constructor Store cannot be invoked without 'new'/)
  })
  ...
  if (!isSSR) {
    ...
    it("watch: getter function has access to store's getters object", (done) => {
      ...
      const spy = jest.spyOn({ getter }, 'getter') as unknown as Function
      const spyCb = jest.fn()
